...
import { ChevronDown, HelpCircle, Package, Truck, CreditCard, RotateCcw } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto px-4 py-20 space-y-24">
      ...
      <section className="text-center space-y-6">
        ...
        <p className="text-gray-500 max-w-sm mx-auto font-medium uppercase tracking-tight text-xs">
          Everything you need to know about the CRAVE. ecosystem. No fluff, just facts.
        </p>
      </section>
        ...
        {faqCategories.map((category, catIdx) => (
          <div key={catIdx} className="space-y-8">
            <div className="flex items-center gap-4 border-b border-eco-gray pb-4">
               ...
               <h2 className="text-2xl font-display font-bold italic tracking-tight">{category.title}</h2>
            </div>
              ...
              {category.questions.map((faq, faqIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <h3 className="font-bold text-sm uppercase tracking-tight text-eco-dark">{faq.q}</h3>
                      <div className={`w-8 h-8 rounded-full bg-eco-gray flex items-center justify-center transition-all ${isOpen ? 'rotate-180 bg-eco-primary text-white' : ''}`}>
                        <ChevronDown size={14} />
                      </div>
                    </button>
      ...
      <div className="bg-eco-dark p-12 rounded-[3.5rem] text-center space-y-6 relative overflow-hidden">
         ...
         <h4 className="text-2xl font-display font-bold italic text-white leading-tight">Still hunting for answers?</h4>
         <p className="text-white/40 text-xs font-bold uppercase tracking-widest max-w-xs mx-auto">Our support team is active 24/7. Reach out via the secure line.</p>
         <a href="/contact" className="inline-block bg-white text-eco-dark px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-[0.2em] hover:bg-eco-primary hover:text-white transition-all shadow-2xl mt-4">
           Direct Line
         </a>
      </div>
    </div>
