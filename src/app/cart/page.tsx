...
export default function CartPage() {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-gray-500">Looks like you haven't added any sustainable goodies yet.</p>
        </div>
        <Link href="/shop" className="inline-block bg-eco-primary text-white px-10 py-4 rounded-full font-bold hover:shadow-xl transition-all">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-12">
      <h1 className="text-5xl font-display font-bold text-eco-dark italic">Cart</h1>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="grow space-y-1 text-center sm:text-left">
                <h3 className="text-xs md:font-bold md:text-sm text-eco-dark">{item.name}</h3>
                <p className="text-eco-primary font-bold text-xs md:text-sm"><span className='text-sm md:text-xl'>৳</span>{item.price.toFixed(2)}</p>
                <div className="flex items-center justify-center sm:justify-start pt-1">
                   <span className="text-[6px] md:text-[8px] md:font-bold uppercase tracking-widest  md:text-slate-400 border border-slate-200 px-1.5 py-0.5 rounded">Verified</span>
                </div>
              </div>
              <div className="flex items-center sm:space-x-4">
                <div className="flex items-center border border-eco-bg rounded-full md:p-1 bg-eco-gray">
                  <button onClick={() => updateQuantity(item.productId, item.quantity - 1)} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center hover:bg-white rounded-full transition-colors">-</button>
          ...
          <div className="bg-eco-dark text-white p-8 rounded-[3rem] space-y-8 shadow-xl">
            ...
            <div className="space-y-4 text-white/60 text-xs font-medium">
              <div className="flex justify-between">
                ...
                <span className="text-white font-bold"><span className='text-xl'>৳</span>{totalAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="uppercase tracking-widest text-[#C8D3C0]">Shipping</span>
              ...
              <div className="pt-4 border-t border-white/10 flex justify-between items-end">
                <div>
                  ...
                  <p className="text-4xl font-display text-white font-bold italic"><span className='text-3xl'>৳</span>{totalAmount.toFixed(2)}</p>
                </div>
              </div>
            </div>
